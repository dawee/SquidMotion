...
class AnimationStore extends Store {
  ...
  createStep(channel, time) {
    this.currentStep = channel.createStep(time);
  ...
  createChannel(animation) {
    ...
    this.createStep(channel, 0);
    return channel;
  }

  createAnimation() {
    const animation = new MotionTimeline(this.initial);
  ...
  onCreateStep(opts) {
    const channel = this.getChannel(opts.channelId);

    this.createStep(channel, opts.time);
    this.emit(`change:channel:${channel.id}`);
    this.emit('change:step:current');
  }

  getStep(channelId, stepId) {
    return this.getChannel(channelId).steps.find(
      (step) => stepId === step.id
    );
  }
  ...
  onSelectStep(opts) {
    ...
    this.time = this.currentStep.time;
    this.emit('change:step:current');
  }
  ...
  onImportSVG(opts) {
    ...
    firstAnimation = this.createAnimation();
