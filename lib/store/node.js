...
class Node {
  ...
  constructor(nodeId, fileId, initial) {
    this.initial = sanitize(initial.attributes);
    this.boundingBox = initial.computeBoundingBox();
...
NodeStore.bindAction(actions.selectNode, (data) => {
  const node = NodeStore.get({fileId: data.fileId, nodeId: data.nodeId});

  if (!node.selected && !data.append) {
    for (const lastSelected of NodeStore.filter({fileId: data.fileId, selected: true}).all()) {
      lastSelected.selected = false;
    }
  }

  node.selected = !node.selected || !data.append;

  NodeStore.emit('change');
});


NodeStore.bindAction(actions.translateSelected, (data) => {
  for (const node of NodeStore.filter({fileId: data.fileId, selected: true}).all()) {
    if (node.currentStep) {
