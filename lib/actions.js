...
actions.selectTime = Dispersive.createAction((fileId, time) => ({fileId, time}));

actions.moveTime = Dispersive.createAction((fileId, timeline, xDelta) => {
  ...
  return Dispersive.createActionGroup()
    .chain(actions.selectTime, [fileId, newTime < 0 ? 0 : newTime]);
});
