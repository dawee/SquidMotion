...
class Cursor extends Squid.Component {
  ...
  onDrag(tx) {
    actions.moveTime(this.props.fileId, this.state.timeline, tx);
  }
  ...
  initState() {
    this.state = {
      left: TimelineStore.getCursorLeft(this.props.fileId),
  ...
  onTimelineChange() {
    this.setState({left: TimelineStore.getCursorLeft(this.props.fileId)});
  }
